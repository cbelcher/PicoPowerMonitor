...
namespace PicoPowerMonitor
{
    ...
    public sealed partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            // debug
            Debug.WriteLine($"string.IsNullOrEmpty(_targetPort), Pico detected on {_targetPort} during initialization.");
                ...
                using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_PnPEntity WHERE Caption LIKE '%(COM%)'"))
                {
                    ...
                    foreach (var port in ports)
                    {
                            ...
                            !string.IsNullOrEmpty(caption))
                        {
                            ...
                            Debug.WriteLine($"Regex.Match(caption): {m}");
                            // if (m.Success) return m.Groups[1].Value;
                            if (m.Success)
                            {
                                var selected = caption.Trim('(', ')');  //Trim('(', ')');
                                var selected1 = m.Groups[1].Value.Trim('(', ')');
                                Debug.WriteLine($"selected variable contains: {selected}");
                                Debug.WriteLine($"m variable contains: {m}");
                                Debug.WriteLine($"m.Group[1].Value is: {m.Groups[1].Value}");
                                Debug.WriteLine($"selected1 variable contains: {selected1}");
            ...
            try
            {
                ...
                if (match.Success)
                {
                    ...
                    DispatcherQueue.TryEnqueue(() => {
                        if (VoltageText != null) VoltageText.Text = $"V: {v} V";
                        if (CurrentText != null) CurrentText.Text = $"I: {i} A";
                        if (PowerText != null) PowerText.Text = $"P: {p} W";
                    });
                }
            }
            catch (Exception)
